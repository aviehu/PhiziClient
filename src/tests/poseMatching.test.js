...
const wantedPoseAngles = [
    { name: 'left_elbow', angle: 90.00000000000001, normal: [1, 0, 0] },
    -1,
    {
        name: 'left_shoulder',
        angle: 3.5319658152567635,
        normal: [-0.064208984375, 0, 0.014079371467232704]
    },
    {
        name: 'right_shoulder',
        angle: 4.110063613604469,
        normal: [0.0655517578125, 0, -0.014771588146686554]
    },
    {
        name: 'left_hip',
        angle: 136.22801154490935,
        normal: [
            0.014435858022807224,
            0.015314088352738509,
            -0.25717151076511224
        ]
    },
    {
        name: 'right_hip',
        angle: 15.692964847916432,
        normal: [
            -0.09765980325755663,
            -0.0022166182670844137,
            -0.009614058100617662
        ]
    },
    {
        name: 'left_knee',
        angle: 111.17490199958557,
        normal: [-0.0214063213143163, 0.007278324802769021, 0.1205000812156356]
    },
    {
        name: 'right_knee',
        angle: 171.571529301973,
        normal: [
            -0.018347312783589587,
            0.0013849608576492756,
            -0.010568132133751185
        ]
    }
]
...
test('isMatchingWithinThreshold_true', () => {
    const expectedResult = { "angle": 90.00000000000001, "name": "left_elbow", "normal": [1, 0, 0] }
    expect(isMatching(wantedPoseAngles, userPose)[0].angle).toBeCloseTo(expectedResult.angle)
    expect(isMatching(wantedPoseAngles, userPose)[0].name).toBe(expectedResult.name)
    expect(isMatching(wantedPoseAngles, userPose)[0].normal).toStrictEqual(expectedResult.normal)
...
test('isMatchingFalse', () => {
    ...
    expect(isMatching(wantedPoseAngles, userPose)).toMatchObject(expectedResult)
})
